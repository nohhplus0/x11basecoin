...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop X11BaseCoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "X11BaseCoin server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    /* X11BaseCoin features */
    { "x11basecoin",               "masternode",             &masternode,             true,      true,       false },
    { "x11basecoin",               "masternodelist",         &masternodelist,         true,      true,       false },
    { "x11basecoin",               "mnbudget",               &mnbudget,               true,      true,       false },
    { "x11basecoin",               "mnbudgetvoteraw",        &mnbudgetvoteraw,        true,      true,       false },
    { "x11basecoin",               "mnfinalbudget",          &mnfinalbudget,          true,      true,       false },
    { "x11basecoin",               "mnsync",                 &mnsync,                 true,      true,       false },
    { "x11basecoin",               "spork",                  &spork,                  true,      true,       false },
#ifdef ENABLE_WALLET
    { "x11basecoin",               "darksend",               &darksend,               false,     false,      true  }, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use x11basecoind, or the -server option to x11basecoin-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> x11basecoin-cli " + methodname + " " + args + "\n";
}
