...
*Setup instructions for a gitian build of X11BaseCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the X11BaseCoin
...
- [Building X11BaseCoin](#building-x11basecoin)
...
Clone the git repositories for x11basecoin and gitian and then checkout the x11basecoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/x11basecoinpay/x11basecoin.git
cd x11basecoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the x11basecoin repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building X11BaseCoin
----------------

To build X11BaseCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the x11basecoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/x11basecoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/x11basecoinpay/x11basecoin
...
For example:
```bash
URL=https://github.com/crowning-/x11basecoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit x11basecoin=${COMMIT} --url x11basecoin=${URL} ../x11basecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit x11basecoin=${COMMIT} --url x11basecoin=${URL} ../x11basecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit x11basecoin=${COMMIT} --url x11basecoin=${URL} ../x11basecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/x11basecoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/x11basecoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/x11basecoin-build.assert
```
...
[x11basecoin/gitian.sigs](https://github.com/x11basecoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
